...
public class AgreementEnforcement {
     ...
     */
    public void enforce(EAgreement agreement, Date since, boolean isLastExecution) {
        ...
        Map<EGuaranteeTerm, IMetricsRetrieverV2.RetrievalItem> retrievalItems = buildRetrievalItems(agreement, since, now, isLastExecution);
     ...
     */
    public void enforce(EAgreement agreement, Map<EGuaranteeTerm, List<IMonitoringMetric>> metricsMap) {
        logger.debug("enforce(agreement={}) ", agreement.getAgreementId());

        checkInitialized(false);
        
        Map<EGuaranteeTerm, GuaranteeTermEvaluationResult> evaluationResult = 
     ...
     */
    public void enforceBusiness(EAgreement agreement,
            Map<EGuaranteeTerm, List<EViolation>> violationsMap) {
    ...
    private boolean guaranteeTermHasToBeEvaluated(EGuaranteeTerm term, boolean isLastExecution){
        boolean hasToBeRetrieved = false;
        if (isLastExecution){
            if (term.getSamplingPeriodFactor()!=null){ 
                hasToBeRetrieved = (term.getSamplingPeriodFactor()== EGuaranteeTerm.ENFORCED_AT_END);
            }
        }else{
            if (term.getSamplingPeriodFactor()!=null){
                if ((term.getLastSampledDate()==null) && (term.getSamplingPeriodFactor()!= EGuaranteeTerm.ENFORCED_AT_END)){
                    hasToBeRetrieved = true;
                }else{
                    if (term.getSamplingPeriodFactor()!= EGuaranteeTerm.ENFORCED_AT_END){
    ...
    private Map<EGuaranteeTerm, IMetricsRetrieverV2.RetrievalItem> buildRetrievalItems(
        ...
        Map<EGuaranteeTerm, IMetricsRetrieverV2.RetrievalItem> retrievalItems = 
                    new HashMap<EGuaranteeTerm, IMetricsRetrieverV2.RetrievalItem>();
        
        for (final EGuaranteeTerm term : agreement.getGuaranteeTerms()) {
            if (guaranteeTermHasToBeEvaluated(term, isLastExecution)){
                ...
                retrievalItems.put(term, new IMetricsRetrieverV2.RetrievalItem() {
                    ...
                    @Override
                    public EGuaranteeTerm getGuaranteeTerm() {
                        return term;
                    }
    ...
    private Map<EGuaranteeTerm, List<IMonitoringMetric>> getMetrics(
            IMetricsRetriever retriever, 
            EAgreement agreement, 
            Map<EGuaranteeTerm, IMetricsRetrieverV2.RetrievalItem> retrievalItems) {
        logger.debug("getMetrics");
        
        Map<EGuaranteeTerm, List<IMonitoringMetric>> result = new HashMap<EGuaranteeTerm, List<IMonitoringMetric>>();
