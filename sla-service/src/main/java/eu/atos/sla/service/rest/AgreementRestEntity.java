...
public class AgreementRestEntity extends AbstractSLARest{
    ...
    @Resource(name = "agreementParser")
    private XmlParser<Agreement> xmlParser;
    
    private AgreementEntityToWsagAdapter adapter;
    ...
    public List<Agreement> getAgreements(
        ...
        List<EAgreement> agreements = agreementRestHelper.getAgreements(
                consumerId, providerId, templateId, BooleanParam.getValue(active));
            
        List<Agreement> result = new TransformingList<EAgreement, Agreement>(
                agreements, getAdapter()
        );
        return result;
    }
    ...
    @Path("{id}")
    @Produces( { MediaType.APPLICATION_XML, MediaType.APPLICATION_JSON } )
    public Agreement  getAgreementById(@PathParam("id") String agreementId) throws NotFoundException {
        logger.debug("StartOf getAgreementById REQUEST for /agreements/" + agreementId);

        AgreementHelperE agreementRestHelper = getAgreementHelper();
        EAgreement agreement =  agreementRestHelper.getAgreementByID(agreementId);
        if (agreement==null){
        ...
        Agreement a = getAdapter().apply(agreement);
    ...
    public Response createAgreement(@Context UriInfo uriInfo,  Agreement agreementParam) throws NotFoundException, ConflictException, InternalException{
        logger.debug("StartOf createAgreement - Insert /agreements");
        String id, location = null;
        try{
            AgreementHelperE agreementRestHelper = getAgreementHelper();
            id = agreementRestHelper.createAgreement(agreementParam);
    ...
    @Path("active")
    @Produces( { MediaType.APPLICATION_XML, MediaType.APPLICATION_JSON } )
    public List<Agreement> getActiveAgreements() throws NotFoundException  {
        logger.debug("StartOf getActiveAgreements - Get active agreements");
        long actualDate = new Date().getTime();
        AgreementHelperE agreementRestHelper = getAgreementHelper();
        List<EAgreement> agreements = agreementRestHelper.getActiveAgreements(actualDate);
        
        List<Agreement> result = new TransformingList<EAgreement, Agreement>(agreements, getAdapter());
    ...
    private Adapter<EAgreement, Agreement> getAdapter() {
        if (adapter == null) {
            adapter = new AgreementEntityToWsagAdapter(xmlParser);
        }
        return adapter;
    }
