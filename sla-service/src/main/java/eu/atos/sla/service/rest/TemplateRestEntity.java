...
import eu.atos.sla.common.collections.TransformingList;
import eu.atos.sla.common.collections.TransformingList.Adapter;
import eu.atos.sla.datamodel.ETemplate;
import eu.atos.sla.modelconversion.TemplateEntityToWsagAdapter;
...
public class TemplateRestEntity extends AbstractSLARest{
    ...
    @Resource(name = "templateParser")
    private XmlParser<Template> xmlParser;
    
    private TemplateEntityToWsagAdapter adapter;
     ...
     */
    @GET
    public List<Template> getTemplates(@QueryParam("providerId") String providerId, @QueryParam("serviceIds") String serviceIds)  {
        ...
        List<ETemplate> templates =  templateRestHelper.getTemplates(providerId, serviceIdsSplitted);
        
        List<Template> result = new TransformingList<ETemplate, Template>(templates, getAdapter());
    ...
    @GET
    @Path("{id}")
    public Template getTemplateByUuid(@PathParam("id") String templateId) throws NotFoundException{
        logger.debug("StartOf getTemplateByUuid - REQUEST for /templates/{}", templateId);
        TemplateHelperE templateRestHelper = getTemplateHelper();
        ETemplate template = templateRestHelper.getTemplateByUUID(templateId);
        if (template==null){
        ...
        Template result = getAdapter().apply(template);
     ...
     */
    @POST
    @Consumes( { MediaType.APPLICATION_XML, javax.ws.rs.core.MediaType.APPLICATION_JSON } )
    public Response createTemplate(@Context UriInfo uriInfo, Template templateParam) throws NotFoundException, ConflictException, InternalException {
        logger.debug("StartOf createTemplate - Insert /templates");

        TemplateHelperE templateRestHelper = getTemplateHelper();
        String id, location = null;
        try {
            id = templateRestHelper.createTemplate(templateParam, "");
    ...
    @PUT
    @Path("{uuid}")
    public ETemplate updateTemplate(@PathParam("uuid") String uuid, TemplateParam templateParam) throws InternalException, NotFoundException, ConflictException{
        logger.debug("StartOf updateTemplate");
        ETemplate updatedTemplate = null;
        try{
            TemplateHelperE templateRestHelper = getTemplateHelper();
    ...
    private Adapter<ETemplate, Template> getAdapter() {
        if (adapter == null) {
            adapter = new TemplateEntityToWsagAdapter(xmlParser);
        }
        return adapter;
    }
}
