...
public class ProviderHelperE {
    ...
    public List<Provider> getProviders() throws HelperException {
        ...
        List<EProvider> storedProviders = providerDAO.getAll();
        List<Provider> providers = new ArrayList<Provider>(); 
        for (EProvider storedProvider:storedProviders) providers.add(modelConverter.getProviderXML(storedProvider));
    ...
    public Provider getProviderByUUID(String uuid)  {
        ...
        EProvider storedProvider = providerDAO.getByUUID(uuid);
    ...
    public String createProvider(Provider providerXML) throws DBExistsHelperException, InternalHelperException {
        ...
        EProvider stored = null;
        if (providerXML != null) {
            if (!doesProviderExistsInRepository(providerXML.getUuid(), providerXML.getName())) {
                EProvider provider = modelConverter.getProviderFromProviderXML(providerXML);
    ...
    public boolean deleteByProviderUUID(String providerUUID) throws DBExistsHelperException{
        ...
        List<ETemplate> listTemplates = templateDAO.search(providerUUID, null);
        if ((listTemplates==null) || listTemplates.size()>0){ 
            ...
            List<EAgreement> listAgreements = agreementDAO.getByProvider(providerUUID);
            if ((listAgreements==null) || listAgreements.size()>0){ 
                ...
                EProvider provider = providerDAO.getByUUID(providerUUID);
                if (provider!=null)
