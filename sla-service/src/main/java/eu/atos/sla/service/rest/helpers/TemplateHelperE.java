...
public class TemplateHelperE  {
    ...
    private EProvider getProviderPerUUIDFromDatabase(String providerUUID) {
        return providerDAO.getByUUID(providerUUID);
    }
    

    public List<ETemplate> getTemplates(String providerId, String []serviceIds){
        logger.debug("StartOf getTemplates providerId {}, serviceIds{}",providerId, serviceIds);
        List<ETemplate> templates = null;
    ...
    public ETemplate getTemplateByUUID(String uuid) {
        logger.debug("StartOf getTemplateByUUID uuid:{}", uuid);
        ETemplate template = templateDAO.getByUuid(uuid);
    ...
    public boolean deleteTemplateByUuid(String uuid) throws DBExistsHelperException  {
        ...
        List<EAgreement> list = agreementDAO.getByTemplate(uuid);
        if (list.size() > 0) {
        ...
        try {
            ...
            if (templateXML != null) {
                ...
                if (!doesTemplateIdExistInDatabase(templateXML.getTemplateId())) {
                    ...
                    EProvider provider = getProviderPerUUIDFromDatabase(providerUUID);
                    
                    if (provider==null) throw new DBMissingHelperException("Provider with UUID "+providerUUID+" doesn't exist in the database");

                    ETemplate template = modelConverter.getTemplateFromTemplateXML(templateXML, originalSerializedTemplate);
    ...
    public ETemplate updateTemplate(String uuid, Template templateXML, String originalSerializedTemplate) 
        ...
        try{
            ...
            List<EAgreement> agreementList = agreementDAO.getByTemplate(uuid);
            ...
            ETemplate templateFromDatabase = null;
