...
public class BusinessValueListParser {
    ...
    protected EBusinessValueList parse(GuaranteeTerm guaranteeTermXML) 
        ...
        EBusinessValueList businessValueList = newBusinessValueList(importance);
    ...
    private void parseCustomBusinessValuesOld(BusinessValueList bvlXml,
            EBusinessValueList businessValueList) throws JAXBException {
        if (bvlXml.getCustomBusinessValue() != null) {
            for (Object bvlItem : bvlXml.getCustomBusinessValue()) {
                ...
                for (Penalty action: item.getPenalties()) {
                    ...
                    EPenaltyDefinition penaltyDef = newPenaltyDefinition(item, action);
    ...
    private void parseCustomBusinessValues(BusinessValueList bvlXml,
            EBusinessValueList businessValueList) {
        if (bvlXml.getCustomBusinessValue() != null) {
    ...
    protected EBusinessValueList newBusinessValueList(int importance) {
        
        Collection<EPenaltyDefinition> emptyList = Collections.<EPenaltyDefinition>emptyList();
        EBusinessValueList businessValueList = 
                new EBusinessValueList(
                        importance, 
                        emptyList
                );
        return businessValueList;
    ...
    protected EPenaltyDefinition newPenaltyDefinition(CustomBusinessValue item, Penalty action) {
        
        EPenaltyDefinition penalty = new EPenaltyDefinition(
                item.getCount(), 
                item.getDuration(), 
                action.getType(), 
                action.getUnit(), 
