...
public class EnforcementJob {
    ...
    public EnforcementJob(EEnforcementJob j) {
        if (j.getAgreement() == null) {
            throw new IllegalArgumentException("Cannot construct EnforcementJob with null agreement");
        }
        this.agreementId = j.getAgreement().getAgreementId();
        this.enabled = j.getEnabled();
        this.lastExecuted = j.getLastExecuted();
    }
